...
The functionality is pretty basic: we set up two inputs, and after clicking on a button we send a request to a server with the data and then show that the operation is successful by getting the id of that person on our client-api server.
...
Next, we should add an event listener to a button that after clicking on it would send the data to our api server.
...
As you can see, there is no rocket science. We just make a post to another server with the data that we got from our client-api.

### Our beloved listener

The server-listener takes a big role in our project - without it, we wouldn't even create our project. This is the script you'd work on if you only had to connect to companies API that is mentioned at the beginning of this tutorial.
